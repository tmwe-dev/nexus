'use strict';
const store=require('../crm/storage/store');
...
async function upsert(input){return store.upsert('companies',normalize(input));}
async function get(id){return store.get('companies',id);}
async function list({search='',limit=50,offset=0}={}){const q=String(search).trim().toLowerCase();let items=await store.list('companies');if(q)items=items.filter(x=>[x.name,x.legal_name,x.vat_number,x.tax_id,x.country,x.city,x.website].some(v=>String(v||'').toLowerCase().includes(q)));return {items:items.slice(offset,offset+limit),total:items.length,limit,offset};}
