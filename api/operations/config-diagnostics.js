...
module.exports=async function handler(req,res){
 ...
 const checks=[
  ...
  ['NEXUS_FUNNEMAIL_URL','Funnemail Endpoint'],
  ['NEXUS_FUNNEMAIL_TOKEN','Funnemail Auth'],
  ['NEXUS_BARTALK_URL','BarTalk Endpoint'],
  ['NEXUS_BARTALK_TOKEN','BarTalk Auth'],
  ['NEXUS_TMWE2_URL','TMWE2 Endpoint'],
  ['NEXUS_TMWE2_TOKEN','TMWE2 Auth'],
 ...
 return res.status(200).json({contract:'operations.config-diagnostics.v3',configured,total:items.length,percent:Math.round(configured/items.length*100),missing:items.filter(x=>!x.configured).map(x=>x.key),items,originals_modified:false});
};