...
module.exports=async function handler(req,res){
 ...
 const checks=[
  ['NEXUS_CRM_DATABASE_URL','Durable CRM Runtime'],
 ...
 return res.status(200).json({contract:'operations.config-diagnostics.v1',configured,total:items.length,percent:Math.round(configured/items.length*100),missing:items.filter(x=>!x.configured).map(x=>x.key),items,originals_modified:false});
};