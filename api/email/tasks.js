...
module.exports=async function handler(req,res){
 ...
 try{
  ...
  if(req.method==='POST'){
   ...
   if(service.configured())return res.status(201).json({...await service.request(req,'/tasks',{method:'POST',body:b}),source:'funnemail-service-boundary'});
   const priority=Number(b.priority)||3;const data=await rest(req,'/rpc/fn_create_task_from_mail',{method:'POST',body:{p_message_id:b.message_id||b.source_message_id,p_title:String(b.title||'(senza titolo)').slice(0,200),p_priority:priority,p_due_at:b.due_at||due(priority),p_auto_reason:b.auto_reason||'manual',p_snapshot:b.snapshot||{}}});return res.status(201).json({contract:'email.task.create.v2',source:'funnemail-compatibility-adapter',data});
  }
