 ...
 try{
  if(req.method==='GET'){
   ...
   if(service.configured()){
    const q=new URLSearchParams({limit:String(limit)});if(status)q.set('status',status);
    const data=await service.request(req,`/drafts?${q.toString()}`);return res.status(200).json({...data,source:'funnemail-service-boundary'});
   }
  ...
  if(req.method==='POST'){
   ...
   const execution=await idempotency.run({req,capability:'email.draft.create.v1',auth:guard.auth,responseStatus:201},async()=>{
    if(service.configured()){const boundary=await service.request(req,'/drafts',{method:'POST',body:payload});return boundary?.data||null;}
   ...
   return res.status(201).json({contract:'email.draft.create.v1',source:service.configured()?'funnemail-service-boundary':'funnemail-compatibility-adapter',data:execution.result,idempotency:{replayed:false,key:execution.ticket.key||null,durable:Boolean(execution.ticket.durable)}});
  }
