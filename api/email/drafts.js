...
module.exports=async function handler(req,res){
 ...
 try{
  if(req.method==='GET'){
   ...
   const data=await rest(req,path);return res.status(200).json({contract:'email.draft.list.v1',source:'funnemail-supabase-adapter',items:data||[]});
  }
  if(req.method==='POST'){
   ...
   const data=await rest(req,'/email_drafts',{method:'POST',headers:{Prefer:'return=representation'},body:payload});return res.status(201).json({contract:'email.draft.create.v2',source:'funnemail-supabase-adapter',data:Array.isArray(data)?data[0]:data});
  }
