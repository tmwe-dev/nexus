...
module.exports=async function handler(req,res){
 ...
 try{
  if(service.configured()){const data=await service.request(req,'/classify',{method:'POST',body:b});return res.status(200).json({...data,source:'funnemail-service-boundary'});}
  const data=await edge(req,'funnemail-classify',b);return res.status(200).json({contract:'email.classify.v1',source:'funnemail-classify',data});
