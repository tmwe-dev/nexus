...
module.exports=async function handler(req,res){
 ...
 try{
  ...
  const execution=await idempotency.run({req,capability:'email.send.v1',auth:guard.auth,responseStatus:200},()=>edge(req,'funnemail-send-direct',payload));
  ...
  return res.status(200).json({contract:'email.send.v1',source:'funnemail-send-direct',data:execution.result,idempotency:{replayed:false,key:execution.ticket.key||null,durable:Boolean(execution.ticket.durable)}});
