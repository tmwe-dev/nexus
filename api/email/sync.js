...
module.exports=async function handler(req,res){
 ...
 try{
  const execution=await idempotency.run({req,capability:'email.sync.v1',auth:guard.auth,responseStatus:200},()=>edge(req,'funnemail-imap-sync',req.body||{}));
  ...
  return res.status(200).json({contract:'email.sync.v1',source:'funnemail-imap-sync',data:execution.result,idempotency:{replayed:false,key:execution.ticket.key||null,durable:Boolean(execution.ticket.durable)}});
