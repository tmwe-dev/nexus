...
module.exports=async function handler(req,res){
 ...
 try{
  if(service.configured()){
   const data=await service.request(req,'/dashboard');
  ...
  return res.status(200).json({contract:'email.dashboard.v1',source:'funnemail-compatibility-adapter',totals:{inbound,unread,today:oggi,drafts:drafts.length,enriched_senders:intel.length},categories,draft_status:draftStatus});
 }catch(error){return res.status(error.status||502).json({error:'FUNNEMAIL_DASHBOARD_UNAVAILABLE',message:error.message,detail:error.detail||null});}
};
