...
module.exports=async function handler(req,res){
 ...
 try{
  if(service.configured()){
   const data=await service.request(req,'/status',{method:'POST',body:{message_id:id,action}});
  ...
  return res.status(200).json({contract:'email.message.status.v1',source:'funnemail-compatibility-adapter',message_id:id,action,ok:true});
